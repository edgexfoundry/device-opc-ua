...
if __name__ == "__main__":
    ...
    ns = server.register_namespace("https://gopcua.com/")

    simulations = server.nodes.objects.add_object(ua.NodeId("simulations", ns), "simulations")
    fnEven = simulations.add_method(ua.NodeId("simulate_connection_failure", ns), "simulate_connection_failure",
                                    simulate_connection_failure, [], [])
    fnEven = simulations.add_method(ua.NodeId("simulate_securechannel_failure", ns), "simulate_securechannel_failure",
                                    simulate_securechannel_failure, [], [])
    fnEven = simulations.add_method(ua.NodeId("simulate_session_failure", ns), "simulate_session_failure",
                                    simulate_session_failure, [], [])
    fnEven = simulations.add_method(ua.NodeId("simulate_subscription_failure", ns), "simulate_subscription_failure",
                                    simulate_subscription_failure, [], [])
